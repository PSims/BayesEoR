...
from BayesEoR.Linalg import IDFT_Array_IDFT_2D_ZM_SH, makeGaussian,\
    ...
    IDFT_Array_IDFT_1D, IDFT_Array_IDFT_1D,\
...
def generate_test_sim_signal(nu, nv, nx, ny, nf, neta):
    ...
    multi_chan_dft_array_noZMchan = block_diag(*[
        dft_array.T for i in range(nf)])
    # Updated for python 3: floor division
    k_z, k_y, k_x = np.mgrid[-(nf//2) : (nf//2),
                             -(nu//2) : (nu//2)+1,
                             -(nv//2) : (nv//2)+1]
    ...
    k_sigma_2 = 10.0
    bin_limit = 2.0
    bin_1_selector_in_model_mask = np.logical_and(bin_limit > mod_k,
                                                  mod_k > 0.0)
    bin_2_selector_in_model_mask = mod_k >= bin_limit

    # Do not include high spatial frequency structure in the
    # data except when performing wn-fitting in the analysis
    Nyquist_k_z_mode = k_z[0, 0, 0]
    Second_highest_frequency_k_z_mode = k_z[-1, 0, 0]
    high_spatial_frequency_selector_mask = np.logical_or(
    ...
    bin_1_selector_in_model_mask = np.logical_and(
        bin_1_selector_in_model_mask,
        high_spatial_frequency_mask)
    bin_2_selector_in_model_mask = np.logical_and(
        bin_2_selector_in_model_mask,
        high_spatial_frequency_mask)

    # It would be more accurate to call this the k_cube_std_scaling_cube
    k_cube_power_scaling_cube[bin_1_selector_in_model_mask] = k_sigma_1
    k_cube_power_scaling_cube[bin_2_selector_in_model_mask] = k_sigma_2
    ...
    bin_1_selector_in_model_mask_and_ZM = np.logical_and.reduce(
        (bin_1_selector_in_model_mask, ZM_mask))
    bin_2_selector_in_model_mask_and_ZM = np.logical_and.reduce(
        (bin_2_selector_in_model_mask, ZM_mask))
    ...
    return s, s_im, k_cube_signal, k_cube_power_scaling_cube,\
           random_k, bin_1_selector_in_model_mask_and_ZM,\
           bin_2_selector_in_model_mask_and_ZM, bin_1_selector_in_model_mask,\
           bin_2_selector_in_model_mask, k_sigma_1, k_sigma_2, bin_limit,\
           high_spatial_frequency_selector_mask, high_spatial_frequency_mask
...
def generate_k_cube_in_physical_coordinates(nu, nv, nx, ny, nf, neta):
    ...
    z, y, x = np.mgrid[-(nf//2) : (nf//2),
                       -(nu//2) : (nu//2)+1,
                       -(nv//2) : (nv//2)+1]
    # A distance of one pixel in the uv-plane corresponds to 2.5 lambda
    uvcellsize = 2.5
    # Comoving distance at redshift 10.26
    DCov = 9745.3730

    # k_x = 2*pi*u/D_M, k_y = 2*pi*v/D_M
    deltakperp = 2.*np.pi * uvcellsize / DCov
...
def generate_k_cube_in_physical_coordinates_21cmFAST(nu, nv, nx, ny, nf, neta):
    ...
    box_size_xy_MyCube_Mpc = 2048.
    box_size_z_MyCube_Mpc = 2048. * nf/box_size_z_21cmFAST_pix

    # Construct k-space arrays
...
def construct_GRN_unitary_hermitian_k_cube(nu, nv, neta, nq):
    ...
    complex_GRN /= complex_GRN.std()

    image_cube_mean = 1.0
    GRN_k_cube_vector = np.hstack(
        (complex_GRN, image_cube_mean, complex_GRN[::-1].conjugate()))
...
def construct_GRN_scaled_hermitian_k_cube(
    ...
    for i_bin in range(len(bin_selector_in_k_cube_mask)):
        if n_kz_even:
            # Ignore unmoddelled Nyquist bin in mask by
            # starting at mask channel 1
            current_vals = complex_GRN[
                bin_selector_in_k_cube_mask[i_bin].flatten()[
                n_kxy:n_kxy+complex_GRN.size]
            ]
            current_std = np.hstack(
                (current_vals, current_vals.conjugate())
                ).std()
            inds = bin_selector_in_k_cube_mask[i_bin].flatten()\
                [n_kxy:n_kxy+complex_GRN.size]
            complex_GRN[inds] *= k_sigma[i_bin] / current_std
        else:
            # Updated for python 3: floor division
            inds = bin_selector_in_k_cube_mask[i_bin].flatten()\
                [:n_kxy//2 + (n_kxy*n_kz_odd)//2]
            current_vals = complex_GRN[inds]
            current_std = np.hstack(
                (current_vals, current_vals.conjugate())
                ).std()
            complex_GRN[inds] *= k_sigma[i_bin] / current_std
    ...
    if n_kz_even:
        # Add in Nyquist channel but give it zero power
        # (since it is currently not part of the data model).
        GRN_k_cube_even = np.zeros([n_kz_odd+1, n_ky,n_kx]) + 0j
        GRN_k_cube_even[1 : n_kz_odd+1, 0 : n_ky, 0 : n_kx] = GRN_k_cube
        GRN_k_cube = GRN_k_cube_even
...
def generate_test_sim_signal_with_large_spectral_scales_2_HERA_Binning(
    ...
    k_sigma = [k_sigma_1, k_sigma_2, k_sigma_3, k_sigma_4,
               k_sigma_5, k_sigma_6, k_sigma_7, k_sigma_8]
    ...
    for i_bin in range(numKbins):
        ...
        n_elements = np.sum(np.logical_and.reduce(
            (mod_k > modkbins[i_bin, 0],
             mod_k <= modkbins[i_bin, 1],
             k_z > 0)))
        if n_elements > 0:
            n_bins += 1
            print(i_bin, modkbins[i_bin, 0], modkbins[i_bin, 1], n_elements)
            total_elements += n_elements
            bin_selector_in_k_cube_mask.append(np.logical_and(
                mod_k > modkbins[i_bin, 0],
                mod_k <= modkbins[i_bin, 1],
                k_z>0))
    ...
    # pectral data since  these terms aren't included in the data model
    Nyquist_k_z_mode = k_z[0, 0, 0]
    Second_highest_frequency_k_z_mode = k_z[-1, 0, 0]
    # Mean_k_z_mode = 0.0
    # NOTE: the k_z=0 term should not necessarily be masked out since
    # it is still required as a quadratic component (and is not
    # currently explicitly added in there) even if it is not used for
    # calculating the power spectrum.
    high_spatial_frequency_selector_mask = np.logical_or.reduce(
        (k_z == Nyquist_k_z_mode, k_z == Second_highest_frequency_k_z_mode))
    high_spatial_frequency_mask = np.logical_not(
        high_spatial_frequency_selector_mask)

    for i_bin in range(len(bin_selector_in_k_cube_mask)):
        bin_selector_in_k_cube_mask[i_bin] = np.logical_and(
            bin_selector_in_k_cube_mask[i_bin], high_spatial_frequency_mask)
    ...
    for i_bin in range(len(bin_selector_in_k_cube_mask)):
        bin_selector_in_k_cube_mask[i_bin] = np.logical_and(
            bin_selector_in_k_cube_mask[i_bin], ZM_mask)

    k_cube_signal = construct_GRN_scaled_hermitian_k_cube(
        nu, nv, neta, nq, bin_selector_in_k_cube_mask, k_sigma)
    ...
    s_quad_only = np.dot(multi_chan_dft_array_noZMchan / (nu*nv)**0.5,
                         s_im_quad_only.flatten())
    s_fourier_only = np.dot(multi_chan_dft_array_noZMchan / (nu*nv)**0.5,
                            s_im_fourier_only.flatten())
    s = np.dot(multi_chan_dft_array_noZMchan / (nu*nv)**0.5, s_im.flatten())


    from scipy.linalg import block_diag
    # Note: the nf**0.5 normalisation factor results in a symmetric
    # transform and is necessary for correctly estimating the power
    # spectrum. However, it is not consistent with Python's asymmetric
    # DFTs, therefore this factor needs to be removed when comparing to
    # np.fft.fftn cross-checks!
    idft_array_1D = IDFT_Array_IDFT_1D(nf, neta+nq) * Fz_normalisation
    multi_vis_idft_array_1D = block_diag(*[
        idft_array_1D for i in range(nu*nv - 1)])

    k2 = np.dot(multi_vis_idft_array_1D.conjugate().T,
                s_fourier_only.reshape(neta+nq, -1).T.flatten())

    return s, s_im, s_quad_only, s_im_quad_only, s_fourier_only,\
           s_im_fourier_only, bin_selector_in_k_cube_mask,\
           high_spatial_frequency_selector_mask, k_cube_signal,\
           k_sigma, ZM_mask, k_z_mean_mask


def generate_test_sim_signal_with_large_spectral_scales_2_21cmFAST_Binning(
        nu, nv, nx, ny, nf, neta, nq):
    # Generate test sim data
    Fz_normalisation = nf**0.5
    DFT2D_Fz_normalisation = (nu*nv*nf)**0.5
    ...
    k_sigma = [k_sigma_1, k_sigma_2, k_sigma_3, k_sigma_4, k_sigma_5,
               k_sigma_6, k_sigma_7, k_sigma_8, k_sigma_9, k_sigma_10]
    ...
    # Do not include high spatial frequency structure in the power
    # spectral data if nq>0, since these terms aren't included
    # in the data model
    Nyquist_k_z_mode = k_z[0, 0, 0]
    Second_highest_frequency_k_z_mode = k_z[-1, 0, 0]
    ...
    # the power spectrum.
    # NOTE 2: bin_selector_in_k_cube_mask is actually only used for
    # working out which parts of the k-cube NOT to use (should change
    # to a name more descriptive of its later usage) when calculating
    # LogDetPhi in the likelihood and hence the k_z=0 term (which should
    # not be in that determinant!) should be masked out here!
    # ...probably.
    Mean_k_z_mode = 0.0
    k_z_mean_mask = k_z != Mean_k_z_mode
    ...
    if nq == 1:
        ...
        high_spatial_frequency_selector_mask =\
            np.zeros(k_z.shape).astype('bool')
    high_spatial_frequency_mask = np.logical_not(
        high_spatial_frequency_selector_mask)
    ...
    for i_bin in range(len(bin_selector_in_k_cube_mask)):
        if nq>0:
            # bin_selector_in_k_cube_mask[i_bin] = np.logical_and(
            # 	bin_selector_in_k_cube_mask[i_bin],
            # 	high_spatial_frequency_mask)
            bin_selector_in_k_cube_mask[i_bin] = np.logical_and(
                bin_selector_in_k_cube_mask[i_bin],
                high_spatial_frequency_mask,
...
def map_out_bins_for_power_spectral_coefficients_HERA_Binning(
        nu, nv, nx, ny, nf, neta, nq, bin_selector_in_k_cube_mask,
        high_spatial_frequency_selector_mask, ZM_mask, k_z_mean_mask):
    ...
    k_z, k_y, k_x = np.mgrid[-(nf//2) : (nf//2),
                             ...
                             -(nv//2) : (nv//2)+1]
    mod_k = (k_z**2. + k_y**2. + k_x**2.)**0.5
    bin_selector_in_k_cube_mask_vis_ordered = [
        bin_selector_in_k_cube_mask[i_bin].T.flatten()
        for i_bin in range(len(bin_selector_in_k_cube_mask))]
    k_perp_3D = (k_x**2. + k_y**2)**0.5
    ZM_mask = k_perp_3D > 0.0
    ...
    if nq > 0:
        ...
        # 	np.logical_and.reduce(high_spatial_frequency_mask_vis_ordered,
        # 						  ZM_2D_mask_vis_ordered,
        # 						  k_z_mean_mask_vis_ordered)
    else:
        ZM_2D_and_high_spatial_frequencies_mask_vis_ordered =\
            ZM_2D_mask_vis_ordered
        # ZM_2D_and_high_spatial_frequencies_mask_vis_ordered = np.logical_and(
        # 	ZM_2D_mask_vis_ordered, k_z_mean_mask_vis_ordered)

    bin_selector_in_model_mask_vis_ordered = [
        bin_selector_in_k_cube_mask_vis_ordered[i_bin][
            ZM_2D_and_high_spatial_frequencies_mask_vis_ordered
        ]
        for i_bin in range(len(bin_selector_in_k_cube_mask_vis_ordered))]

    return bin_selector_in_model_mask_vis_ordered


def map_out_bins_for_power_spectral_coefficients_WQ_v2_HERA_Binning(
        nu, nv, nx, ny, nf, neta, nq, bin_selector_in_k_cube_mask,
        high_spatial_frequency_selector_mask, ZM_mask, k_z_mean_mask):
    # Map out bins for power spectral coefficients
    bin_selector_in_model_mask_vis_ordered =\
        map_out_bins_for_power_spectral_coefficients_HERA_Binning(
            nu, nv, nx, ny, nf, neta, nq,
            bin_selector_in_k_cube_mask, high_spatial_frequency_selector_mask,
            ZM_mask, k_z_mean_mask)

    bin_selector_in_model_mask_vis_ordered_reshaped = [
        bin_selector_in_model_mask_vis_ordered[i_bin].reshape(-1, neta)
        for i_bin in range(len(bin_selector_in_model_mask_vis_ordered))]

    WQ_boolean_array = np.zeros(
        [bin_selector_in_model_mask_vis_ordered_reshaped[0].shape[0], nq]
        ).astype('bool')
    bin_selector_in_model_mask_vis_ordered_reshaped_WQ = [
        np.hstack((
            bin_selector_in_model_mask_vis_ordered_reshaped[i_bin],
            WQ_boolean_array))
        for i_bin in range(len(
                bin_selector_in_model_mask_vis_ordered_reshaped))]

    bin_selector_in_model_mask_vis_ordered_WQ = [
        bin_selector_in_model_mask_vis_ordered_reshaped_WQ[i_bin].flatten()
        for i_bin in range(len(
            bin_selector_in_model_mask_vis_ordered_reshaped_WQ))]

    k_z_mean_mask_vis_ordered = np.logical_not(k_z_mean_mask).T[
        np.logical_and(ZM_mask.T,
                       np.logical_not(high_spatial_frequency_selector_mask).T)]
    k_z_mean_mask_vis_ordered_reshaped = k_z_mean_mask_vis_ordered.reshape(
        bin_selector_in_model_mask_vis_ordered_reshaped[0].shape)

    Quad_modes_only_boolean_array_vis_ordered = np.hstack(
        (k_z_mean_mask_vis_ordered_reshaped.astype('bool'),
         np.logical_not(WQ_boolean_array))
        ).flatten()

    return bin_selector_in_model_mask_vis_ordered_WQ,\
           Quad_modes_only_boolean_array_vis_ordered


def map_out_bins_for_power_spectral_coefficients(
        nu, nv, nx, ny, nf, neta, nq, bin_1_selector_in_k_cube_mask,
        bin_2_selector_in_k_cube_mask, bin_3_selector_in_k_cube_mask,
        bin_4_selector_in_k_cube_mask, bin_5_selector_in_k_cube_mask,
        bin_6_selector_in_k_cube_mask, high_spatial_frequency_selector_mask):
    # Map out bins for power spectral coefficients
    # Updated for python 3: floor division
    k_z, k_y, k_x = np.mgrid[-(nf//2) : (nf//2),
                             -(nu//2) : (nu//2)+1,
    ...
    bin_1_selector_in_k_cube_mask_vis_ordered =\
        bin_1_selector_in_k_cube_mask.T.flatten()
    bin_2_selector_in_k_cube_mask_vis_ordered =\
        bin_2_selector_in_k_cube_mask.T.flatten()
    bin_3_selector_in_k_cube_mask_vis_ordered =\
        bin_3_selector_in_k_cube_mask.T.flatten()
    bin_4_selector_in_k_cube_mask_vis_ordered =\
        bin_4_selector_in_k_cube_mask.T.flatten()
    bin_5_selector_in_k_cube_mask_vis_ordered =\
        bin_5_selector_in_k_cube_mask.T.flatten()
    bin_6_selector_in_k_cube_mask_vis_ordered =\
        bin_6_selector_in_k_cube_mask.T.flatten()
    ...
    bin_1_selector_in_model_mask_vis_ordered =\
        bin_1_selector_in_k_cube_mask_vis_ordered[
            ZM_2D_and_high_spatial_frequencies_mask_vis_ordered]
    bin_2_selector_in_model_mask_vis_ordered =\
        bin_2_selector_in_k_cube_mask_vis_ordered[
            ZM_2D_and_high_spatial_frequencies_mask_vis_ordered]
    bin_3_selector_in_model_mask_vis_ordered =\
        bin_3_selector_in_k_cube_mask_vis_ordered[
            ZM_2D_and_high_spatial_frequencies_mask_vis_ordered]
    bin_4_selector_in_model_mask_vis_ordered =\
        bin_4_selector_in_k_cube_mask_vis_ordered[
            ZM_2D_and_high_spatial_frequencies_mask_vis_ordered]
    bin_5_selector_in_model_mask_vis_ordered =\
        bin_5_selector_in_k_cube_mask_vis_ordered[
            ZM_2D_and_high_spatial_frequencies_mask_vis_ordered]
    bin_6_selector_in_model_mask_vis_ordered =\
        bin_6_selector_in_k_cube_mask_vis_ordered[
            ZM_2D_and_high_spatial_frequencies_mask_vis_ordered]

    return bin_1_selector_in_model_mask_vis_ordered,\
           bin_2_selector_in_model_mask_vis_ordered,\
           bin_3_selector_in_model_mask_vis_ordered,\
           bin_4_selector_in_model_mask_vis_ordered,\
           bin_5_selector_in_model_mask_vis_ordered,\
...
def map_out_bins_for_power_spectral_coefficients_WQ_v2(
        nu, nv, nx, ny, nf, neta, nq, bin_1_selector_in_k_cube_mask,
        bin_2_selector_in_k_cube_mask, bin_3_selector_in_k_cube_mask,
        bin_4_selector_in_k_cube_mask, bin_5_selector_in_k_cube_mask,
        bin_6_selector_in_k_cube_mask, high_spatial_frequency_selector_mask):
    ...
    high_spatial_frequency_selector_mask = \
        map_out_bins_for_power_spectral_coefficients(
            nu, nv, nx, ny, nf, neta, nq,
            bin_1_selector_in_k_cube_mask,
            bin_2_selector_in_k_cube_mask,
            bin_3_selector_in_k_cube_mask,
            bin_4_selector_in_k_cube_mask,
            bin_5_selector_in_k_cube_mask,
            bin_6_selector_in_k_cube_mask,
            high_spatial_frequency_selector_mask)

    bin_1_selector_in_model_mask_vis_ordered_reshaped =\
        bin_1_selector_in_model_mask_vis_ordered.reshape(-1,neta)
    bin_2_selector_in_model_mask_vis_ordered_reshaped =\
        bin_2_selector_in_model_mask_vis_ordered.reshape(-1,neta)
    bin_3_selector_in_model_mask_vis_ordered_reshaped =\
        bin_3_selector_in_model_mask_vis_ordered.reshape(-1,neta)
    bin_4_selector_in_model_mask_vis_ordered_reshaped =\
        bin_4_selector_in_model_mask_vis_ordered.reshape(-1,neta)
    bin_5_selector_in_model_mask_vis_ordered_reshaped =\
        bin_5_selector_in_model_mask_vis_ordered.reshape(-1,neta)
    bin_6_selector_in_model_mask_vis_ordered_reshaped =\
        bin_6_selector_in_model_mask_vis_ordered.reshape(-1,neta)

    WQ_boolean_array = np.zeros([
        bin_1_selector_in_model_mask_vis_ordered_reshaped.shape[0], nq]
        ).astype('bool')
    bin_1_selector_in_model_mask_vis_ordered_reshaped_WQ =\
        np.hstack((bin_1_selector_in_model_mask_vis_ordered_reshaped,
                   WQ_boolean_array))
    bin_2_selector_in_model_mask_vis_ordered_reshaped_WQ =\
        np.hstack((bin_2_selector_in_model_mask_vis_ordered_reshaped,
                   WQ_boolean_array))
    bin_3_selector_in_model_mask_vis_ordered_reshaped_WQ =\
        np.hstack((bin_3_selector_in_model_mask_vis_ordered_reshaped,
                   WQ_boolean_array))
    bin_4_selector_in_model_mask_vis_ordered_reshaped_WQ =\
        np.hstack((bin_4_selector_in_model_mask_vis_ordered_reshaped,
                   WQ_boolean_array))
    bin_5_selector_in_model_mask_vis_ordered_reshaped_WQ =\
        np.hstack((bin_5_selector_in_model_mask_vis_ordered_reshaped,
                   WQ_boolean_array))
    bin_6_selector_in_model_mask_vis_ordered_reshaped_WQ =\
        np.hstack((bin_6_selector_in_model_mask_vis_ordered_reshaped,
                   WQ_boolean_array))

    bin_1_selector_in_model_mask_vis_ordered_WQ =\
        bin_1_selector_in_model_mask_vis_ordered_reshaped_WQ.flatten()
    bin_2_selector_in_model_mask_vis_ordered_WQ =\
        bin_2_selector_in_model_mask_vis_ordered_reshaped_WQ.flatten()
    bin_3_selector_in_model_mask_vis_ordered_WQ =\
        bin_3_selector_in_model_mask_vis_ordered_reshaped_WQ.flatten()
    bin_4_selector_in_model_mask_vis_ordered_WQ =\
        bin_4_selector_in_model_mask_vis_ordered_reshaped_WQ.flatten()
    bin_5_selector_in_model_mask_vis_ordered_WQ =\
        bin_5_selector_in_model_mask_vis_ordered_reshaped_WQ.flatten()
    bin_6_selector_in_model_mask_vis_ordered_WQ =\
        bin_6_selector_in_model_mask_vis_ordered_reshaped_WQ.flatten()
    ...
    return bin_1_selector_in_model_mask_vis_ordered_WQ,\
           bin_2_selector_in_model_mask_vis_ordered_WQ,\
           bin_3_selector_in_model_mask_vis_ordered_WQ,\
           bin_4_selector_in_model_mask_vis_ordered_WQ,\
           bin_5_selector_in_model_mask_vis_ordered_WQ,\
...
def generate_k_cube_model_spherical_binning_v2d0(
        mod_k_masked, k_z_masked, nu, nv, nx, ny, nf, neta, nq):
    # Generate k_cube physical coordinates
    # to match the 21cmFAST input simulation
    mod_k, k_x, k_y, k_z, deltakperp, deltakpara, x, y, z =\
