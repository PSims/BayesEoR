...
class DataUnitConversionmKAndJyPerPix:
    ...
    def Brightness_RJ(self, Frequency_Hz, Temperature_K, **kwargs):
        ...
        B= (2*(Nu**2) * K * T) / c**2
        if Print:
            print('B = ', B, 'W m^-2 Hz^-1 sr^-1')
        return B

    def convert_from_mK_to_Jy_per_pix(
            self, Data_mK, Channel_Frequencies_Array_Hz,
            Pixel_Height_rad, **kwargs):
        # Will need to be updated for HEALPix pixel functions
        Data_K = Data_mK / 1.e3
        ...
        for i_channel in range(len(Channel_Frequencies_Array_Hz)):
            ...
            Temperature_K = 1.0
            Image_Conversion_Factor_K_to_Jy_per_Pixel = (
                    Pixel_Area_sr
                    * self.Brightness_RJ(Channel_Frequency_Hz,
                                         Temperature_K,
                                         Print=False)
                    * 1.e26)
            Data_Jy_per_Pixel[i_channel] = (
                    Data_K[i_channel]
                    * Image_Conversion_Factor_K_to_Jy_per_Pixel)
...
class Cosmology:
    ...
    def Calculate_Comoving_Distance_Mpc_Between_Redshifts_z1_and_z2(
        ...
        self.Comoving_Distance_Mpc, self.Comoving_convergence_uncertainty =\
            integrate.quad(self.Comoving_Distance_Mpc_Integrand,self.z1,self.z2)
    ...
    def Convert_from_Tau_to_Kz(self, Tau_Array, **kwargs):
        self.K_z_Array = (
                ((2.*pi*self.H_0*self.f_21*self.E_z2) /
                 (self.c_km_per_sec*(1.+self.z2)**2.)) *
                Tau_Array)
...
class ExtractDataFrom21cmFASTCube():
    ...
    def calculate_box_size_in_degrees_and_MHz(self, Box_Side_cMpc,
                                              simulation_redshift, **kwargs):
        cosmo = Cosmology(z2=simulation_redshift, Print=1)
        bandwidth_MHz = (
            cosmo.Convert_from_Comoving_Mpc_to_Delta_Frequency_at_Redshift_z2(
                Box_Side_cMpc=Box_Side_cMpc))
        central_frequency_MHz = (
            Cosmology().Convert_from_Redshift_to_21cmFrequency(
                simulation_redshift))
        output_21cmFast_box_width_deg = (
            cosmo.Convert_from_Comoving_Mpc_to_Delta_Angle_at_Redshift_z2(
                Box_Side_cMpc=Box_Side_cMpc))

        if self.Print:
            print('bandwidth_MHz', bandwidth_MHz)
        if self.Print:
            print('central_frequency_MHz', central_frequency_MHz)
        if self.Print:
            print('output_21cmFast_box_width_deg',
                  output_21cmFast_box_width_deg)

        return bandwidth_MHz, central_frequency_MHz,\
               output_21cmFast_box_width_deg
...
class WriteDataToFits(ExtractDataFrom21cmFASTCube):
    ...
    def write_data(self, Data, output_path, **kwargs):
        ...
        # ===== Inputs =====
        self.Box_Side_cMpc = kwargs.pop('Box_Side_cMpc',
                                        default_Box_Side_cMpc)
        self.simulation_redshift = kwargs.pop('simulation_redshift',
                                              default_simulation_redshift)

        nz, ny, nx = Data.shape
        bandwidth_MHz, central_frequency_MHz, output_21cmFast_box_width_deg =\
            self.calculate_box_size_in_degrees_and_MHz(
                self.Box_Side_cMpc, self.simulation_redshift)
        ...
        hdulist[0].header.set('CDELT1',
                              -output_21cmFast_box_width_deg/float(nx))
        ...
        hdulist[0].header.set('CDELT2',
                              output_21cmFast_box_width_deg/float(ny))
        ...
        hdulist[0].header.set(
            'CRVAL3',
            (central_frequency_MHz - (subset_bandwidth_MHz/2.)) * 1.e6)
        hdulist[0].header.set('CDELT3',
                              (subset_bandwidth_MHz/float(nz)) * 1.e6)
