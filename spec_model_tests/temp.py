...
if p.include_instrumental_effects:
	if overwrite_data_with_WN:
		...
		if p.use_EoR_cube:
			...
			effective_noise = generate_visibility_covariance_matrix_and_noise_realisation_and_the_data_vector_instrumental_v1(1.0*sigma, s_EoR, nu,nv,nx,ny,nf,neta,nq,random_seed=EoR_noise_seed)[1].std()
...
#--------------------------------------------
# Calculate (un-normalised) power spectrum of the EoR subset cube
#--------------------------------------------
...
if sub_MLLWM:
	if not iterative_sub_MLLWM:
		...
		if p.use_LWM_Gaussian_prior:
			...
			PSPP_block_diag.inverse_LW_power = 1./9.e9
