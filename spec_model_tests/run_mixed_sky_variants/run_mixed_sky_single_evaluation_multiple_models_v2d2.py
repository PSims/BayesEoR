...
if Continue:
	...
	b1_b2_logL_array_noise_i_total, sigma, Tb_experimental_std_K, beta_experimental_mean = np.load('random/b1_b2_logL/{}/b1_b2_logL_array_noise_i_total_sigma_{}_nu_{}_nv_{}_multiple_noise_realisations.npy'.format(max_min_dir, sigma, 15, 15))
	...
	plot_Bayes_factor_uncertainty_ratio = True
	if plot_Bayes_factor_uncertainty_ratio:
		b1_b2_logL_array_noise_i_total, sigma, Tb_experimental_std_K, beta_experimental_mean = np.load('random/b1_b2_logL/{}/b1_b2_logL_array_noise_i_total_sigma_{}_nu_{}_nv_{}.npy'.format(max_min_dir, sigma, 15, 15))
		...
		pylab.figure(figsize=(10,10))
		plot_extent = [b1_b2_logL_array_sorted[:,0][0],b1_b2_logL_array_sorted[:,0][-1],b1_b2_logL_array_sorted[:,1][-1],b1_b2_logL_array_sorted[:,1][0]]
		pylab.imshow(abs(b1_b2_logL_noise_i_array_uncertainties/b1_b2_logL_noise_i_array[0]), extent=plot_extent, vmax=1.0)
		pylab.colorbar(fraction=(0.315*0.15), pad=0.01)
		# uncertainty_contours = [1.0, 2.0, 3.0]
		# CS = pylab.contour(b1_b2_logL_array_sorted[:,1].reshape(nb,nb), b1_b2_logL_array_sorted[:,0].reshape(nb,nb), (b1_b2_logL_noise_i_array_uncertainties), uncertainty_contours, alpha=1.0, colors='r', linestyles='--')
		pylab.xlabel('Power law index, $b_{1}$', fontsize=16)
		...
		fig_dir = 'Plots/model_evidence_b1_b2_v1d0/{}/'.format(max_min_dir)
		pylab.tight_layout()
		pylab.savefig(fig_dir+'model_evidence_b1_b2_sample_uncertainties_div_single_field_bayes_factors_sigma_{}'.format(sigma).replace('.','d')+'.png')
		pylab.show()
