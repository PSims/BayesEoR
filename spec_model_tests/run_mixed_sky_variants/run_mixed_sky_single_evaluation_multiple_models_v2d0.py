...
# 	def renormalise_matrices_for_scaled_noise(self, T_Ninv_T, T, block_T_Ninv_T, Ninv, **kwargs):
# 		if self.have_run_calc_matrix_renormalisation_scale_factor:
# 			print "About to update likelihood matrices to from a constructed noise level of {} to a newly assumed noise in the data of {} using a scale factor sigma_init**2./updated_sigma**2. = {}".format()
# 		else:
# 			print "Warning, the calc_matrix_renormalisation_scale_factor hasn't been used to derive this updated matrix_renormalisation_scale_factor so the output noise level isn't being tracked by this class...\nUpdating likelihood matrices with the manually input scale factor: {}...".format()

# 		print 'Renormalising T_Ninv_T'
# 		T_Ninv_T       = T_Ninv_T*self.matrix_renormalisation_scale_factor
# 		print 'Renormalising T'
# 		T              = T*self.matrix_renormalisation_scale_factor
...
# 		return T_Ninv_T, T, block_T_Ninv_T, Ninv
...
if use_GDSE_foreground_cube:
	...
	hour_angle_HERA_fields_subset = hour_angle[20::60] #Define a new field every 10 degrees starting at an hour angle of ~13.75 (which leads to a good spread of SI and stds to test).
	std_array_HERA_fields_subset = std_array[20::60]
	SI_vals_HERA_fields_subset = SI_vals[20::60] 

	hour_angle_HERA_fields_subset = hour_angle_HERA_fields_subset[np.logical_not(np.isnan(std_array_HERA_fields_subset))] #Don't consider fields in the excluded region (where the field enters within +- 20 deg of the Galactic plane)
	std_array_HERA_fields_subset = std_array_HERA_fields_subset[np.logical_not(np.isnan(std_array_HERA_fields_subset))] 
	SI_vals_HERA_fields_subset = SI_vals_HERA_fields_subset[np.logical_not(np.isnan(SI_vals_HERA_fields_subset))] 
...
for noise_i in range(1):
	...
	if use_EoR_cube:
		...
		d = generate_visibility_covariance_matrix_and_noise_realisation_and_the_data_vector_v2(sigma, fg_GDSE, nu,nv,nx,ny,nf,neta,nq, random_seed=1234)[0]
		s_Tot = fg_GDSE.copy()
		s_fgs_only = fg_GDSE.copy()
		print 'Using GDSE cube'
		if use_freefree_foreground_cube:
...
if Continue:
	...
	if sigma==10.:
		...
		pylab.imshow(b1_b2_logL_noise_i_array[0], extent=plot_extent, vmin=-10.0)
	if sigma==30.:
