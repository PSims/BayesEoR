...
b1_b2_logL_array_sorted_dict = {}
for p.beta_experimental_mean in np.linspace(2.5,2.63,14):
# for p.beta_experimental_mean in [2.5,2.51]:
	
	print p.beta_experimental_mean
	...
	for b1 in np.linspace(2.0,3.5,16):
		for b2 in np.linspace(2.0,3.5,16):
			if b1 <= b2:
	...
	print p.beta_experimental_mean
	b1_b2_logL_array_sorted_dict[p.beta_experimental_mean] = b1_b2_logL_array_sorted
...
print b1_b2_logL_array_sorted_dict
np.save('random/full_sky_model_optimization/b1_b2_logL_array_sorted_dict.npy', b1_b2_logL_array_sorted_dict)
...
b1_b2_logL_array_sorted_dict = np.load('random/full_sky_model_optimization/b1_b2_logL_array_sorted_dict.npy').tolist()

maxmaxL = -1.e10
for key in b1_b2_logL_array_sorted_dict.keys():
	maxL = b1_b2_logL_array_sorted_dict[key][:,2][np.logical_not(np.isnan(b1_b2_logL_array_sorted_dict[key][:,2]))].max()
	print 'maxL', maxL
	if maxL>maxmaxL:
		maxmaxL=maxL
		print key, maxmaxL, b1_b2_logL_array_sorted_dict[key][:,2][np.logical_not(np.isnan(b1_b2_logL_array_sorted_dict[key][:,2]))].argmax()

print 'maxmaxL', maxmaxL


b1_b2_logL_array_sorted_dict_copy = b1_b2_logL_array_sorted_dict.copy()



for key in b1_b2_logL_array_sorted_dict_copy.keys():
	b1_b2_logL_array_sorted_dict_copy[key][:,2][np.logical_not(np.isnan(b1_b2_logL_array_sorted_dict_copy[key][:,2]))] = b1_b2_logL_array_sorted_dict[key][:,2][np.logical_not(np.isnan(b1_b2_logL_array_sorted_dict[key][:,2]))] - maxmaxL




b1_b2_logL_array_sorted_best_model_comparison = b1_b2_logL_array_sorted_dict_copy[2.6].copy()

for key in b1_b2_logL_array_sorted_dict_copy.keys():
	for i in range(len(b1_b2_logL_array_sorted_best_model_comparison)):
		if b1_b2_logL_array_sorted_best_model_comparison[:,2][i] > b1_b2_logL_array_sorted_dict[key][:,2][i]:
				b1_b2_logL_array_sorted_best_model_comparison[i][2] = b1_b2_logL_array_sorted_dict[key][i][2]
...
if Plot:
	import pylab
	plot_extent = [b1_b2_logL_array_sorted_best_model_comparison[:,0][0],b1_b2_logL_array_sorted_best_model_comparison[:,0][-1],b1_b2_logL_array_sorted_best_model_comparison[:,1][-1],b1_b2_logL_array_sorted_best_model_comparison[:,1][0]]
	pylab.close('all')
	pylab.figure(figsize=(10,10))
	pylab.imshow(b1_b2_logL_array_sorted_best_model_comparison[:,2].reshape(16,16), extent=plot_extent, vmin=-5.)
	pylab.colorbar(fraction=(0.315*0.15), pad=0.01)

	levels = [-1.0, -2.0, -3.0]
	CS = pylab.contour(b1_b2_logL_array_sorted_best_model_comparison[:,1].reshape(16,16), b1_b2_logL_array_sorted_best_model_comparison[:,0].reshape(16,16), (b1_b2_logL_array_sorted_best_model_comparison[:,2].reshape(16,16)), levels[::-1], alpha=1.0, colors='r')
	...
	pylab.savefig(fig_dir+'model_evidence_b1_b2_full_sky_no EGS.png')
	pylab.show()
